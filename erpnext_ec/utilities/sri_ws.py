...
def build_xml_from_dict(data):
	...
	root = ET.Element("RespuestaAutorizacionComprobante")
	_build(root, data)
...
</soapenv:Envelope>""".format(xml_string)
	headers = {"Content-Type": "text/xml"}
	return requests.post(api_url, data=body, verify=False, stream=True, headers=headers, timeout=60)
